document.addEventListener('DOMContentLoaded', function() {
    let form = document.querySelector("#FormCuenta");
    form.addEventListener("change", ()=>{
        for(var i=0;i<form.elements.length;i++){
            var FormC = form.elements[i];
...
function Submit(){
    if(!HTMLObjectsIdAreInDOM(["new-pass", "pass"]))
        return swal({
            title: "Datos perdidos",
            type: "warning",
            text: "Asegúrate de no haber modificado ningún nombre del documento",
            confirmButtonColor: '#3085d6',
            confirmButtonText: 'Aceptar'
        });
    let new_pass = document.querySelector("#new-pass");
    let pass = document.querySelector("#pass");
    let password_is_safe = passwordIsSafe(new_pass.value);
    if(password_is_safe.description){
        return swal({
            title: "Insegura",
            type: "warning",
            text: password_is_safe.description,
            confirmButtonColor: '#3085d6',
            confirmButtonText: 'Aceptar',
            onAfterClose: ()=>{
                new_pass.className += " error";
                new_pass.focus();
            }
        });
    }
    const check_password = ajaxRequest("post", "GuardarPassword", {pass: pass.value, new_pass: new_pass.value});
    check_password.then(obj => console.log(obj));
